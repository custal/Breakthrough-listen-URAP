...
MAX_DATA_ARRAY_SIZE = 2048 * 2048 * 2048 * 2 # Max size of data array to load into memory
...
#                         dd = dd[::-1]
    ...
    def plot_spectrum(self, t=0, f_start=None, f_stop=None, logged=False, if_id=0, c=None, **kwargs):
        ...
        #added to remove middle coarse channel spike artifacts
        n_coarse_chan = self.calc_n_coarse_chan()
        self.blank_dc(n_coarse_chan)
        ...
        if isinstance(t, int):
            ...
            if len(plot_data.shape) > 1:
                plot_data = np.ma.mean(plot_data, axis=0)
            else:
                plot_data = np.ma.mean(plot_data)
        else:
...
#return plot data as it useful for some of my programs
        if logged:
            return plot_f, db(plot_data)
        else:
            return plot_f, plot_data
    ...
    def plot_spectrum_min_max(self, t=0, f_start=None, f_stop=None, logged=False, if_id=0, c=None, **kwargs):
        ...
        #Since the data has been squeezed, the axis for time goes away if only one bin, causing a bug with axis=1
        if len(plot_data.shape) > 1 and self.whichnan == True:
            plot_max = np.nanmax(plot_data, axis=0)
            plot_min = np.nanmin(plot_data, axis=0)
            plot_data = np.nanmean(plot_data, axis=0)
        elif self.whichnan == True:
            plot_max = np.nanmax(plot_data)
            plot_min = np.nanmin(plot_data)
    ...
    def plot_time_series(self, f_start=None, f_stop=None, if_id=0, logged=True, orientation='h', MJD_time=False, **kwargs):
        ...
        #Since the data has been squeezed, the axis for time goes away if only one bin, causing a bug with axis=1
        #Use nanmean if data has been cut
        if len(plot_data.shape) > 1 and self.whichnan == True:
